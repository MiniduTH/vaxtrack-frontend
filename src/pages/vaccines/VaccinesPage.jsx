    ...
    <div className="p-6 sm:p-10 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center">
            <FiLayers className="mr-3 text-primary-600 dark:text-primary-400" />
            Vaccine Catalog
          </h1>
          <p className="text-secondary-500 dark:text-slate-400 text-sm mt-1">Manage the master list of available vaccines and their schedules.</p>
        </div>
            ...
            <div key={vax._id || vax.id} className="bg-card rounded-xl shadow-sm border border-border overflow-hidden hover:shadow-md transition-shadow group flex flex-col">
              <div className="h-48 bg-secondary-100 dark:bg-slate-800 relative overflow-hidden">
                {vax.imageUrl ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-secondary-400 dark:text-slate-500">
              ...
              <div className="p-5 flex-1 flex flex-col">
                ...
                <p className="text-sm text-secondary-600 dark:text-slate-400 line-clamp-2 mt-auto">
                  {vax.description}
                </p>
              </div>
            </div>
          ...
          <table className="min-w-full divide-y divide-border">
            <thead className="bg-secondary-50 dark:bg-slate-800/50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-secondary-500 dark:text-slate-400 uppercase tracking-wider">Manufacturer</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-secondary-500 dark:text-slate-400 uppercase tracking-wider">Dosage Rules</th>
              ...
              {vaccines.map((vax) => (
                <tr key={vax._id || vax.id} className="hover:bg-gray-50">
                    ...
                    <div className="flex items-center">
                      <div className="h-10 w-10 flex-shrink-0 bg-secondary-100 dark:bg-slate-800 rounded-md overflow-hidden flex items-center justify-center">
                         {vax.imageUrl ? (
                           ...
                           <FiImage className="text-secondary-400 dark:text-slate-500" />
                         )}
                      </div>
                      <div className="ml-4">
                        <div className="text-sm font-bold text-foreground">{vax.name}</div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {vax.dosesRequired > 1 && (
                      <div className="text-xs text-secondary-500 dark:text-slate-400">{vax.daysBetweenDoses} days apart</div>
                    )}
                  </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900 dark:bg-slate-950 bg-opacity-50 backdrop-blur-sm">
          <div className="bg-card rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
              ...
              <form id="vaccine-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-border border-dashed rounded-xl bg-secondary-50 dark:bg-slate-800/50 hover:bg-gray-100 transition relative">
                    <div className="space-y-1 text-center">
                      {imagePreview ? (
                        ...
                        <FiImage className="mx-auto h-12 w-12 text-secondary-400 dark:text-slate-500" />
                      )}
                      ...
                      <p className="text-xs text-secondary-500 dark:text-slate-400">PNG, JPG, WEBP up to 5MB</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-secondary-700 dark:text-slate-300 mb-1">Vaccine Name *</label>
                    <input
                      type="text"
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-secondary-700 dark:text-slate-300 mb-1">Manufacturer *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-secondary-700 dark:text-slate-300 mb-1">Doses Required *</label>
                    <input
                      type="number"
                      min="1"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-secondary-700 dark:text-slate-300 mb-1">Days Between Doses</label>
                    <input
                      type="number"
                      min="0"
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-secondary-700 dark:text-slate-300 mb-1">Description</label>
                    <textarea
                      rows={4}
            ...
            {/* Footer */}
            <div className="p-6 border-t border-border flex justify-end space-x-3 bg-secondary-50 dark:bg-slate-800/50">
              <button
                type="button"
                onClick={closeModal}
